...
export default {
  ...
  methods: {
    async fetchForecast(lat, lon) {
      const res = await getWeatherForecastForPosition(lat, lon);
  ...
  mounted() {
    navigator.geolocation.getCurrentPosition(({coords}) => this.fetchForecast(coords.latitude, coords.longitude));
  }
}
</script>
